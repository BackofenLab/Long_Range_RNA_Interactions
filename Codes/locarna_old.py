...
def main_locarna(param_df_path, cm_path, output_path,
    ...
    for seq_class in seq_dir:
        ...
        make_locarna_fasta(seq_dir[seq_class], fasta_file_name, CDS_left, CDS_right)
        run_mlocarna(fasta_file_name, f"{output_path}/{seq_class}")
        run_rnaalifold(f"{output_path}/{seq_class}/results", seq_dir[seq_class])
